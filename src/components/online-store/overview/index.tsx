...
const OnlineStoreOverview = () => {
  ...
  useEffect(() => {
    axios.get("/api-admin/themes/my-templates").then(({ data }) => {
      setCurrentTemplate(data.current_template)
      setTemplates(data.templates)
    })
  }, [refresh])
  const changeCurrentTheme = (id: string) =>{
    axios.post('/api-admin/themes/use-template',{
        "template_id": id
    }).then(()=>{
        setRefresh(!refresh)
        ...
        {templates.map((template) => (
          ...
          >
            ...
            <div className="flex gap-3">
              ...
              <div className="cursor-pointer rounded border border-gray-300 px-4 py-2 text-black transition-all hover:bg-gray-200"
